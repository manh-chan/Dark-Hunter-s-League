using System;
using System.Collections.Generic;
using UnityEngine;

[Serializable]
public class AchivementData
{
    public bool checkAchivement;
    public bool checkAchivementNewbie;
    public bool checkAchivementFirtsRound;
    public bool checkAchivementProficient;
    public bool checkAchivementSurvivor;
    public bool checkAchivementGodhunt;
    //thuc hien nhiem vu de nhan dc thanh tuu
   
public AchivementData()
    {

    }
    public AchivementData(bool checkAchivement)
    {
        this.checkAchivement = checkAchivement;
    }
}
